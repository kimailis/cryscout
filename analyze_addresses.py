    ...
    for index, row in df.iterrows():
        ...
        if not result:
            ...
            if bc_data:
                ...
                first_date = "Unknown"
                if n_tx > 0:
                    tx_list = bc_data.get('tx')
                    if n_tx <= 50 and tx_list:
                        first_tx = tx_list[-1]
                        ...
                        if bc_data_first and bc_data_first.get('tx'):
                ...
                if tx_list:
                     last_tx_date = time.strftime('%Y-%m-%d', time.gmtime(tx_list[0]['time'])) if 'time' in tx_list[0] else "Unknown"
